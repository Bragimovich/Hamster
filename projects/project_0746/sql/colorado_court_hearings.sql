CREATE TABLE IF NOT EXISTS `crime_inmate`.`colorado_court_hearings` (
  ...
  CONSTRAINT `fk_il_tazewell__court_hearings_il_tazewell__charges12`
    FOREIGN KEY (`charge_id`)
    REFERENCES `crime_inmate`.`colorado_charges` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
