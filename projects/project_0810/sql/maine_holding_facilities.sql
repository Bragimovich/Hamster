CREATE TABLE IF NOT EXISTS `crime_inmate`.`maine_holding_facilities` (
  ...
  CONSTRAINT `fk_il_tazewell__holding_facilities_il_tazewell__arrests810`
    FOREIGN KEY (`arrest_id`)
    REFERENCES `crime_inmate`.`maine_arrests` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
