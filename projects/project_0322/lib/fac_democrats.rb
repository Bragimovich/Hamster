...
require_relative '../models/us_dept_facd'

class FacDemocrats <  Hamster::Scraper
  ...
  def initialize
    ...
    @already_processed = FACD.pluck(:external_link)
    @data_array = []
  end
  ...
  def main
    page_no = 1
    while true
      page_no == 1 ? data_source_url = MAIN_PAGE : data_source_url = "#{MAIN_PAGE}page/#{page_no.to_s}/"
      document, status = connect_to(data_source_url)
      releases = document.css(".post")
      break if releases.empty?
      releases.each do |release|
        ex_link = release.css("a").first['href']
        next if @already_processed.include? ex_link
        document, status = connect_to(ex_link)
        link_parser(document, release, data_source_url, ex_link)
      end
      FACD.insert_all(@data_array) unless @data_array.empty?
      break if @data_array.count < releases.count and page_no > 1
      @data_array = []
      page_no += 1
    end
  end
  ...
  def link_parser(document, release, data_source_url, ex_link)
    ...
    title = release.css("a").first.text.squish
    link = document.css(".category").first["href"]
