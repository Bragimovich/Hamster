CREATE TABLE IF NOT EXISTS `crime_inmate`.`nj_essex_inmate_aliases` (
  ...
  CONSTRAINT `fk_il_tazewell__arrestee_aliases_il_tazewell__arrestees825`
    FOREIGN KEY (`immate_id`)
    REFERENCES `crime_inmate`.`nj_essex_inmates` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
