CREATE TABLE IF NOT EXISTS `crime_inmate`.`nj_essex_holding_facilities` (
  ...
  CONSTRAINT `fk_il_tazewell__holding_facilities_il_tazewell__arrests825`
    FOREIGN KEY (`arrest_id`)
    REFERENCES `crime_inmate`.`nj_essex_arrests` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_holding_facilities_holding_facilities_addresses825`
    FOREIGN KEY (`holding_facilities_addresse_id`)
    REFERENCES `crime_inmate`.`nj_essex_holding_facilities_addresses` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
