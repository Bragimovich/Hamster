...
class ParserClass < Hamster::Parser
  ...
  def get_inner_links(outer_page)
    ...
    links = outer_page.css('#col_json_result a').map{|a| a['href']}
  end

  def get_outer_records(outer_page)
    outer_page = Nokogiri::HTML(outer_page)
    records = outer_page.css('#col_json_result li') 
  end
  ...
  def process_outer_record(record)
    ...
    type = record.css("p.collection-result__date").first.text.strip.downcase rescue 'press release'
    return [title, link, date, type]
  end
  ...
  def fetch_article(file_content)
    article_doc = Nokogiri::HTML(file_content).css("div.entry-content")
  ...
  def fetch_teaser(article, dirty_news)
    ...
    article.children.each do |node|
      ...
      if (node.text.squish[-10..].include? "." or node.text.squish[-10..].include? ":") and node.text.squish.length > 100
