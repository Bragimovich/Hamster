...
class Manager < Hamster::Scraper
  ...
  def download
    ...
    blank_count = 0
    (index..).each do |index|
      bookingnumber = "#{year}-000#{"#{index}".rjust(5,'0')}"
      cookie = get_cookie
      url_search = 'https://portal-ilchampaign.tylertech.cloud/JailSearch/JailingSearch.aspx?ID=400'
      ...
      url_profile = "https://portal-ilchampaign.tylertech.cloud/JailSearch/#{profile_links[0]['href']}"
  ...
  def req_body(viewstate, viewstategenerator, eventvalidation, bookingnumber)
    ...
    req_body += "NodeID=#{url_decode('98100,98200,98400')}&"
  ...
  def headers_search(cookie)
    {
      "Host" => "portal-ilchampaign.tylertech.cloud",
      "Cookie" => cookie
    }
  end
  ...
  def headers_search_post(cookie)
    {
      "Host" => "portal-ilchampaign.tylertech.cloud",
      "Cookie" => cookie,
      "Referer" => "https://portal-ilchampaign.tylertech.cloud/JailSearch/JailingSearch.aspx?ID=400"
    }
  end
