...
require_relative '../models/us_dept_dos_boeapa'
require_relative '../models/us_dept_dos_boeapa_tags'
require_relative '../models/us_dept_dos_boeapa_tags_article_links'
require_relative '../lib/parser'

class ScraperClass < Hamster::Scraper
  
  DOMAIN = 'https://www.state.gov'
  SUB_PATH = '/bureau-of-east-asian-and-pacific-affairs-releases/?results=100'
  SUB_FOLDER = 'east_asian_affairs'
  ...
  def connect_to(url)
    retries = 0
    begin
      response = Hamster.connect_to(url: url, proxy_filter: @proxy_filter, headers: headers)
