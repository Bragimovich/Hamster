...
class  CDCWeeklyCountsOfDeathsByJurisdictionAndAgeGroupScraper < Hamster::Harvester
  CREATED_BY = 'Sergii Butrymenko'
  # SOURCE_FILE = 'Weekly_counts_of_deaths_by_jurisdiction_and_age_group.csv'
  SOURCE_FILE = 'Weekly_Counts_of_Deaths_by_Jurisdiction_and_Age.csv'
  ...
  def initialize
    super
    @url = 'https://data.cdc.gov/api/views/y5bj-9g5w/rows.csv?accessType=DOWNLOAD&bom=true&format=true'
  ...
  def parse_csv_file
    ...
    CDCWeeklyCountsOfDeathsByJurisdictionAndAgeGroup.where(in_csv: 1).update_all(in_csv: 0)
    ...
    CSV.foreach(@file_path, headers: true, converters: [:date, :numeric]) do |row|
      ...
      t = CDCWeeklyCountsOfDeathsByJurisdictionAndAgeGroup.new
      total_rows += 1
      ...
      t.age_group = row[CSV_HEADERS[5]]

      t.number_of_deaths = row[CSV_HEADERS[6]].nil? ? 'Suppressed' : row[CSV_HEADERS[6]].to_s.gsub(',', '')
      t.time_period = row[CSV_HEADERS[7]]
      t.data_type = row[CSV_HEADERS[8]]
      ...
      t.dataset_name_prefix = 'cdc_weekly_counts_of_deaths_by_jurisdiction_and_age_group'
      t.scrape_status = 'live'
      t.pl_gather_task_id = nil
      t.run_id = run_id
      t.in_csv = 1

      db_row = CDCWeeklyCountsOfDeathsByJurisdictionAndAgeGroup.find_by(week_ending_date: t.week_ending_date,
    ...
    deleted_rows = CDCWeeklyCountsOfDeathsByJurisdictionAndAgeGroup.where(in_csv: 0, deleted_at: nil).count
    CDCWeeklyCountsOfDeathsByJurisdictionAndAgeGroup.where(in_csv: 0, deleted_at: nil).update_all(deleted_at: Date.today)
  ...
  def get_last_scraped
    CDCWeeklyCountsOfDeathsByJurisdictionAndAgeGroup.maximum(:last_scrape_date)
  end

  def get_max_run_id
    CDCWeeklyCountsOfDeathsByJurisdictionAndAgeGroup.maximum(:run_id)
  end
  ...
  def check_record_equality(new_row, row)
    ...
    new_row.age_group == row.age_group &&
