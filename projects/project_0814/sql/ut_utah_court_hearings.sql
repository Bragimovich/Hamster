CREATE TABLE IF NOT EXISTS `crime_inmate`.`ut_utah_court_hearings` (
  ...
  CONSTRAINT `fk_il_tazewell__court_hearings_il_tazewell__charges814`
  ...
  CONSTRAINT `fk_court_hearings_court_addresses1`
    ...
    REFERENCES `crime_inmate`.`ut_utah_court_addresses` (`court_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
