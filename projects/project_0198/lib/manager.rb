...
class Manager < Hamster::Scraper
  def initialize
    ...
    @parser = Parser.new
    @sources = [
      {url: 'https://www.hsgac.senate.gov/media/majority-news/', subfolder: 'majority'},
      {url: 'https://www.hsgac.senate.gov/media/minority-news/', subfolder: 'minority'}
    ]
  end
  ...
  def store
    @sources.each do |source|
      subfolder = source[:subfolder]
      run_id = subfolder == 'majority' ? @keeper.get_run_majority : @keeper.get_run_minority
      index = 1
      ...
      files.each do |file|
        ...
        if subfolder == 'majority'
          next if info[:date].to_s < '2023-01-20'
          @keeper.add_info_majority(info, run_id, index) unless info.blank?
        else
          next if info[:date].to_s < '2022-12-23'
          @keeper.add_info_minority(info, run_id, index) unless info.blank?
        end
        index += 1
