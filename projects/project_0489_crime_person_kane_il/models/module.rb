module ModelsHelpers
  ...
  class IlValidateMd5Hash < ActiveModel::Validator
    def validate(record)
      ...
      record.errors.add :md5_hash, "Dublicate md5_hash #{class_validate.class}!" if !find_record.nil?
    end
  end

  class IlValidateArrestsMd5Hash < ActiveModel::Validator
    def validate(record)
      v_str = record.gen_md5
      find_record = IlLakeArrests.find_by(md5_hash: v_str)
      if !find_record.nil?
        record.errors.add :md5_hash, "Dublicate md5_hash Arrest!"
      end
    end
  end
  ...
  class IlValidateArresteeAddressesMd5Hash < ActiveModel::Validator
    def validate(record)
      v_str = record.gen_md5
      find_record = IlLakeArresteeAddresses.find_by(md5_hash: v_str)
      if !find_record.nil?
        record.errors.add :md5_hash, "Dublicate md5_hash!"
      end
    end
