CREATE TABLE IF NOT EXISTS `crime_inmate`.`minnesota_inmate_statuses` (
  ...
  `md5_hash` VARCHAR (255)GENERATED ALWAYS AS (md5(concat_ws('',status,CAST(date_of_status_change AS CHAR),inmate_id))) STORED,
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `fk_immate_statuses_immates816`
