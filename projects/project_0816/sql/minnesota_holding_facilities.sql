CREATE TABLE IF NOT EXISTS `crime_inmate`.`minnesota_holding_facilities` (
  ...
  `md5_hash` VARCHAR (255)GENERATED ALWAYS AS (md5(concat_ws('',CAST(max_release_date as CHAR),CAST(planned_release_date as CHAR),facility,arrest_id))) STORED,
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `fk_il_tazewell__holding_facilities_il_tazewell__arrests816`
