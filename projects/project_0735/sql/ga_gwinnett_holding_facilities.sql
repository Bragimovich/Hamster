CREATE TABLE IF NOT EXISTS `ga_gwinnett_holding_facilities` (
  ...
  CONSTRAINT `fk_il_tazewell__holding_facilities_il_tazewell__arrests1`
    ...
    REFERENCES `crime_inmate`.`ga_gwinnett_arrests` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_holding_facilities_holding_facilities_addresses1`
    FOREIGN KEY (`holding_facilities_addresse_id`)
    REFERENCES `crime_inmate`.`ga_gwinnett_holding_facilities_addresses` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
