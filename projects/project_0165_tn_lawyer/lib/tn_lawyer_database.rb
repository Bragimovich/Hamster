...
def insert_all(lawyers)
  TennesseeLawyerStatus.insert_all(lawyers)
end

def get_existing_bar_numbers(bar_numbers)
  existing_bar_number_array = []
  lawyers = TennesseeLawyerStatus.where(bar_number:bar_numbers)
...
def put_new_touched_id(md5_hash_array, run_id)
  lawyers = TennesseeLawyerStatus.where(md5_hash:md5_hash_array)
...
def deleted_for_not_equal_run_id(run_id)
  lawyers = TennesseeLawyerStatus.where.not(touched_run_id:run_id)
  lawyers.update_all(deleted:1)
end