...
(
  ...
  `touched_run_id`            int,
  `data_source_url`           VARCHAR(255)      DEFAULT 'http://nevadareportcard.nv.gov/di/main/demoprof',
  `source_updated_date`       DATE, 
  `scrape_frequency`          VARCHAR(255) DEFAULT 'Yearly',
  `md5_hash`                  varchar(255) GENERATED ALWAYS AS (md5(CONCAT_WS('', CAST(general_id as CHAR), school_year, subgroup, demographic, count, percent))) STORED,
  `run_id`                    INT,
