CREATE TABLE IF NOT EXISTS `crime_inmate`.`ut_salt_lake_inmate_aliases` (
  ...
  CONSTRAINT `fk_il_tazewell__arrestee_aliases_il_tazewell__arrestees820`
    FOREIGN KEY (`immate_id`)
    REFERENCES `crime_inmate`.`ut_salt_lake_inmates` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
