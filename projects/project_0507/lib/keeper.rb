...
class Keeper < Hamster::Scraper
  ...
  def save_to_aws(link)
    key_start = "crime_perps_mugshots/il/peoria/"
    ...
    file_name += '.png'
    aws_link = nil
    FileUtils.mkdir_p("/home/hamster/HarvestStorehouse/project_0507/store/screens")
    if browser.body.include?('<img')
      browser.screenshot(path: "/home/hamster/HarvestStorehouse/project_0507/store/screens/#{file_name}", selector: 'img')
      key = key_start + file_name
      body = File.open("/home/hamster/HarvestStorehouse/project_0507/store/screens/#{file_name}")
      aws_link = @s3.put_file(body, key, metadata={ url: link })
      logger.info "+ PHOTO LOAD IN AWS!".green
      FileUtils.mv("/home/hamster/HarvestStorehouse/project_0507/store/screens/#{file_name}", "/home/hamster/HarvestStorehouse/project_0507/trash/#{file_name}")
    end
    browser.quit
    aws_link
  rescue => e
  ...
  def get_booking_numbers(year)
    Facilities.connection.execute("select distinct booking_number from il_peoria__holding_facilities as hf join il_peoria__arrests as a on a.id = hf.arrest_id where actual_release_date is null and year(booking_date) != #{year}")
  end
end
