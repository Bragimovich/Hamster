...
class Parser < Hamster::Parser
  ...
  def page_parse(page)
    ...
    if address.blank?
      ...
      address_arr = address.to_s.split('<br xmlns:publicaccessuser="urn:PublicAccessUser">')
      address_arr = address_arr.map{|item| Nokogiri::HTML.parse(item).text.strip}.reject(&:blank?)
      if address_arr.length > 1
        city_state_zip = address_arr[-1].split(',')
        city = city_state_zip[0].strip
        state = city_state_zip[1].strip.split(' ')[0].strip
        zip = city_state_zip[1].strip.split(' ')[1].strip
        street_address = address_arr[0..-2].join(', ').squeeze(' ').strip
      else
        city_state_zip = address_arr[0].split(',')
        street_address = nil
        city = city_state_zip[0].strip
        state = city_state_zip[1].strip.split(' ')[0].strip
        zip = city_state_zip[1].strip.split(' ')[1].strip
      end
      address = address_arr.join(', ').squeeze(' ').strip
    end
    photos = main_content.css('table')[1].css('tr')[0].css('td')[-1].css('img').to_a
    photos = photos.map{|img| "https://justice.peoriacounty.gov/#{img['src']}"}
