...
class  CDCWeeklyCountsOfDeathsByJurisdictionRaceHispanicOriginScraper < Hamster::Harvester
  CREATED_BY = 'Sergii Butrymenko'
  SOURCE_FILE = 'Weekly_Counts_of_Deaths_by_Jurisdiction__and_Race_and_Hispanic_Origin.csv'
  # SOURCE_FILE = 'Weekly_counts_of_deaths_by_jurisdiction_and_race_and_Hispanic_origin.csv'
  ...
  def initialize
    super
    @url = 'https://data.cdc.gov/api/views/qfhf-uhaa/rows.csv?accessType=DOWNLOAD&bom=true&format=true'
  ...
  def parse_csv_file
    ...
    CDCWeeklyCountsOfDeathsByJurisdictionRaceHispanicOrigin.where(in_csv: 1).update_all(in_csv: 0)
    ...
    CSV.foreach(@file_path, headers: true, converters: [:date, :numeric]) do |row|
      ...
      t = CDCWeeklyCountsOfDeathsByJurisdictionRaceHispanicOrigin.new
      total_rows += 1
      ...
      t.mmwryear = row[CSV_HEADERS[3]]
      t.mmwrweek = row[CSV_HEADERS[4]]
      t.race_ethnicity = row[CSV_HEADERS[5]]
      t.time_period = row[CSV_HEADERS[6]]
      t.suppress = row[CSV_HEADERS[7]]
      t.note =  row[CSV_HEADERS[8]]
      t.outcome = row[CSV_HEADERS[9]]
      t.number_of_deaths = row[CSV_HEADERS[10]].nil? ? '' : row[CSV_HEADERS[10]].to_s.gsub(',', '')
      ...
      t.dataset_name_prefix = 'cdc_weekly_counts_of_deaths_by_jurisdiction_race_hispanic_origin'
      t.scrape_status = 'live'
      t.pl_gather_task_id = nil
      t.run_id = run_id
      t.in_csv = 1

      db_row = CDCWeeklyCountsOfDeathsByJurisdictionRaceHispanicOrigin.find_by(week_ending_date: t.week_ending_date,
    ...
    deleted_rows = CDCWeeklyCountsOfDeathsByJurisdictionRaceHispanicOrigin.where(in_csv: 0, deleted_at: nil).count
    CDCWeeklyCountsOfDeathsByJurisdictionRaceHispanicOrigin.where(in_csv: 0, deleted_at: nil).update_all(deleted_at: Date.today)
  ...
  def get_last_scraped
    CDCWeeklyCountsOfDeathsByJurisdictionRaceHispanicOrigin.maximum(:last_scrape_date)
  end

  def get_max_run_id
    CDCWeeklyCountsOfDeathsByJurisdictionRaceHispanicOrigin.maximum(:run_id)
  end
  ...
  def check_record_equality(new_row, row)
    new_row.jurisdiction == row.jurisdiction &&
        ...
        # new_row.mmwryear == row.mmwryear &&
        # new_row.mmwrweek == row.mmwrweek &&
        new_row.race_ethnicity == row.race_ethnicity &&
