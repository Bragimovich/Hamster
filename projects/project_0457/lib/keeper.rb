...
class Keeper
  ...
  def fix_empty_touched_run_id
    ...
    safe_operation {
      @model.where(touched_run_id: nil).update_all(touched_run_id: @run_id - 1) if @run_id > 1
    }
    #TODO add run_id: nil || 0
    safe_operation {
      @model.where(run_id: 0).update_all(run_id: @run_id - 1, touched_run_id: @run_id - 1) if @run_id > 1
    }
  end
