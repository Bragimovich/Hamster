...
class Parser < Hamster::Parser
  ...
  def set_params(**params)
    ...
    @html = params[:html] || @html if @is_browser
  ...
  def elements_list(**params)
    ...
    data = html.css(@css)&.map.with_index do |elem, index|
      ...
      when 'link'
        elem = @converter.string_to_link(elem.attribute(@attribute).to_s, @url_prefix)
      when 'date'
  ...
  def values_after_comments(data, arr_comments, border = nil)
    ...
    values[-1] = values[-1].split(border).first if border
    values
  end
