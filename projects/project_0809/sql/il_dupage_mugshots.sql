CREATE TABLE IF NOT EXISTS `crime_inmate`.`il_dupage_mugshots` (
  ...
  `md5_hash` VARCHAR (255)GENERATED ALWAYS AS (md5(concat_ws('',aws_link,original_link,notes,inmate_id))) STORED,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `md5` (`md5_hash` ASC),
  INDEX `run_id` (`run_id` ASC),
  INDEX `touched_run_id` (`touched_run_id` ASC),
  INDEX `deleted` (`deleted` ASC),
  INDEX `fk_il_tazewell__mugshots_il_tazewell__arrestees809_idx` (`inmate_id` ASC),
  CONSTRAINT `fk_il_tazewell__mugshots_il_tazewell__arrestees809`
