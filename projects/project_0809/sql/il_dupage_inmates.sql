CREATE TABLE IF NOT EXISTS `crime_inmate`.`il_dupage_inmates` (
  ...
  `md5_hash` VARCHAR (255)GENERATED ALWAYS AS (md5(concat_ws('',full_name,first_name,middle_name,last_name,suffix,CAST(birthdate AS CHAR),CAST(date_of_death AS CHAR),sex,race))) STORED,
  PRIMARY KEY (`id`),
