CREATE TABLE IF NOT EXISTS `crime_inmate`.`il_dupage_bonds` (
  ...
  `md5_hash` VARCHAR (255) GENERATED ALWAYS AS (md5(concat_ws('',bond_category,bond_number,bond_type,bond_amount,paid,bond_fees,paid_status,made_bond_release_date,made_bond_release_time,arrest_id,charge_id))) STORED,
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `fk_il_tazewell__bonds_il_tazewell__bonds809`
    FOREIGN KEY (`arrest_id`)
    REFERENCES `crime_inmate`.`il_dupage_arrests` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_il_tazewell__bonds_il_tazewell__arrests809`
