...
class PacerParser
  ...
  def initialize(html, url, run_id, status, case_id, case_party_class, case_activities_class, case_info_class, court)
    @html = html
    @url = url
    @status = status
    @run_id = run_id
    ...
    @case_party_class = case_party_class
    @case_activities_class = case_activities_class
    @case_info_class = case_info_class
    @court_id = court.court_id
    @dev_name = court.author
  ...
  def save_case_info
    case_title            = @case_title.to_s.strip
    case_id               = @case_id.to_s.strip
    case_date             = @case_date
    case_type             = @case_type.to_s.strip
    case_description      = @case_type == CIVIL ? @case_desc.to_s.strip : ''
    disposition_or_status = @disposition_or_status.to_s.strip
    status_as_of_date     = @status.strip
    judge                 = @judge.to_s.strip
    ...
    existing_case_info = @case_info_class.find_by(md5_hash: md5, deleted: false)
    if existing_case_info.nil?
      case_info = @case_info_class.new
      ...
      case_info.data_source_url       = "https://ecf.#{@court_abbr}.uscourts.gov/cgi-bin/DktRpt.pl?#{@url}"
      ...
      begin
        @case_info_class.where(["md5_hash = :md5_hash and deleted = :deleted", { md5_hash: md5, deleted: false }]).update(
  ...
  def save_party(party)
    case_id                = @case_id.to_s.strip
    party_name             = party[:party_name].to_s.strip
    party_type             = party[:party_type].to_s.strip
    party_address          = ''
    party_city             = ''
    party_state            = ''
    party_zip              = ''
    law_firm               = ''
    lawyer_additional_data = ''
    party_description       = @case_type == CRIMINAL ? party[:case_description].to_s.strip : ''
    is_lawyer              = false
    ...
    existing_party = @case_party_class.find_by(md5_hash: md5, deleted: false)
    if existing_party.nil?
      case_party = @case_party_class.new
      ...
      case_party.data_source_url        = "https://ecf.#{@court_abbr}.uscourts.gov/cgi-bin/DktRpt.pl?#{@url}"
      ...
      begin
        @case_party_class.where(["md5_hash = :md5_hash and deleted = :deleted", { md5_hash: md5, deleted: false }]).update(touched_run_id: @run_id)
      rescue StandardError => e
      end
    end
  end
  ...
  def save_lawyer(party)
    case_id                = @case_id.to_s.strip
    party_name             = party[:lawyer].to_s.strip
    party_type             = party[:party_type].to_s.strip + " Lawyer"
    law_firm               = party[:lawyer_firm].to_s.strip
    lawyer_additional_data = party[:lawyer_additional_data].to_s.strip
    is_lawyer              = true
    party_description       = ''

    begin
      lawyer_data   = parse_lawyer_data(lawyer_additional_data)
      party_address = lawyer_data[:address].to_s.strip
      party_city    = lawyer_data[:city].to_s.strip
      party_state   = lawyer_data[:state].to_s.strip
      party_zip     = lawyer_data[:zip].to_s.strip
    rescue StandardError => e
      party_address = ''
      party_city    = ''
      party_state   = ''
  ...
  def save_activities
    @activities.each do |activity|
      ...
      activity_decs = activity[:activity_decs].to_s.strip
      activity_pdf  = activity[:activity_pdf].to_s.strip
      ...
      existing_activity = @case_activities_class.find_by(md5_hash: md5, deleted: false)
      if existing_activity.nil?
        case_activity = @case_activities_class.new
        ...
        case_activity.data_source_url  = "https://ecf.#{@court_abbr}.uscourts.gov/cgi-bin/DktRpt.pl?#{@url}"
        ...
        begin
          @case_activities_class.where(["md5_hash = :md5_hash and deleted = :deleted", { md5_hash: md5, deleted: false }]).update(touched_run_id: @run_id)
        rescue StandardError => e
        end
      end
    end
